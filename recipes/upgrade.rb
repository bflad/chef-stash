...
template "#{node['stash']['install_path']}/bin/setenv.sh Post-Upgrade" do
  source "setenv.sh.erb"
  owner  node['stash']['run_user']
  mode   "0755"
end

template "#{node['stash']['install_path']}/conf/server.xml Post-Upgrade" do
...
template "#{node['stash']['install_path']}/conf/web.xml Post-Upgrade" do
  source "web.xml.erb"
  owner  node['stash']['run_user']
  mode   "0644"
end

template "#{node['stash']['install_path']}/stash-config.properties Post-Upgrade" do
